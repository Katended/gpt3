...
export const fetchChatResponse = createAsyncThunk('chat/fetchChatResponse', async message => {
  const falcon = await client('https://huggingfaceh4-falcon-chat.hf.space');
  return new Promise((resolve, reject) => {
    let timeoutId;
    falcon.submit(1, [message, [], '', 0.8, 0.9]).on('data', evt => {
      if (timeoutId) {
...
const chatSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(fetchChatResponse.pending, (state, action) => {
      state.loading = true;
      state.error = null;
      state.messages.push({
        type: 'user-message',
